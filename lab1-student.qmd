...
## Creating a Plot for Tooth Growth Dataset
...
```{r}
results <- t.test(len ~ supp, data= ToothGrowth, var.euqal = FALSE, alternative = "two.sided")
print(results)
```
1. A P-value of 0.06063 is greater than 0.05 which means we fail to reject the null hypothesis. This suggests there is no statistically significant difference between the affect OJ and VC have on tooth growth.

2. We are 95% confident that the true difference between oj and vc is between -0.1710156 and 7.5710156
